...
def build_dict(img_path, gt_path):
    img_name = img_path.rstrip().split('/')[-1].split('_')[0:3]
    # print('img_name:', img_name)
    gt_name = gt_path.rstrip().split('/')[-1].split('_')[0:3]
...
if __name__ == '__main__':
    ...
    for split in configs['Split']:
        info_list = list()
        Save_paths[split] = os.path.join('', '{}.odgt'.format(split))
        ...
        print(os.path.join(gt_root, split, '*', '*{}'.format(configs['gt_no'])))
