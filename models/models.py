...
class SegmentationModuleBase(nn.Module):
    ...
    def pixel_acc(self, pred, label):  # pred 就是预测的
...
class SegmentationModule(SegmentationModuleBase):
    def __init__(self, net_enc, net_dec, crit, deep_sup_scale=None):
        ...
        self.crit = crit  # 计算 loss
        self.deep_sup_scale = deep_sup_scale

    def forward(self, feed_dict, *, segSize=None):
        if segSize is None: # training
            # 深度监督机制
            # use deep supervision technique
...
class ModelBuilder():
    ...
    def build_encoder(self, arch='resnet50_dilated8', fc_dim=512, weights=''):
        ...
        # this time student net use this
        if arch = 'resnet18_dilated8_stu':
            orig_resnet = resnet.__dict__['resnet18_stu']
            net_encoder = ResnetDilated(orig_resnet, dilate_scale=8)

        # this time teacher net use this  
            ...
            # raise NotImplementedError
            orig_resnet = resnet.__dict__['resnet34'](pretrained=False)
            net_encoder = ResnetDilated(orig_resnet,
                                        dilate_scale=8)
...
class ResnetDilated(nn.Module):
    def __init__(self, orig_resnet, dilate_scale=8):
        ...
        if dilate_scale == 8:
            ...
            # 用途：当一个函数的参数存在于一个元组或者一个字典中时，
            # 用来间接的调用这个函数，并肩元组或者字典中的参数按照顺序传递给参数
    ...
    def forward(self, x, return_feature_maps=False):
        ...
        #print([x.shape for x in conv_out])
        if return_feature_maps:
            return conv_out
        return [x]
...
# DeepSup means Deep Supervision technique
...
class PPMBilinearDeepsup(nn.Module):
    def __init__(self, num_class=150, fc_dim=4096,
        ...
        for scale in pool_scales:
            self.ppm.append(nn.Sequential(
                # AdaptiveAvgPool2d 的输入planes和输出planes是一样的 
                # scale 是输出的尺寸，WxH ，单个单数就是正方形
                nn.AdaptiveAvgPool2d(scale),  
                nn.Conv2d(fc_dim, 512, kernel_size=1, bias=False),
                SynchronizedBatchNorm2d(512),
                nn.ReLU(inplace=True)
    ...
    def forward(self, conv_out, segSize=None):
        ...
        for pool_scale in self.ppm:
            ppm_out.append(nn.functional.upsample(
                ...
                mode='bilinear', align_corners=False))
            #print('in for')
            #print(ppm_out[-1].shape)
        ppm_out = torch.cat(ppm_out, 1) # cat()就是按照不同的方向拼接tensor
